<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="休假天数" prop="xiujiatianshu">
          <el-input v-model="ruleForm.xiujiatianshu" 
              placeholder="休假天数" clearable  :readonly="ro.xiujiatianshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="休假天数" prop="xiujiatianshu">
              <el-input v-model="ruleForm.xiujiatianshu" 
                placeholder="休假天数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="休假开始日期" prop="xiujiakaishiriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.xiujiakaishiriqi" 
                type="date"
                placeholder="休假开始日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xiujiakaishiriqi" label="休假开始日期" prop="xiujiakaishiriqi">
              <el-input v-model="ruleForm.xiujiakaishiriqi" 
                placeholder="休假开始日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiujiatianshu = true;
          ...
          if(o=='xiujiakaishiriqi'){
            this.ruleForm.xiujiakaishiriqi = obj[o];
	    this.ro.xiujiakaishiriqi = true;
            continue;
          }
	...
	this.ruleForm.shenqingyuanyin = this.ruleForm.shenqingyuanyin.replace(reg,'../../../ssm7w5x3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiujiashenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiujiashenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
