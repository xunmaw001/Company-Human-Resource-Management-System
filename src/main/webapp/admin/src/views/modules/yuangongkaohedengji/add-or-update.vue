    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="考核内容" prop="kaoheneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.kaoheneirong" label="考核内容" prop="kaoheneirong">
                    <span v-html="ruleForm.kaoheneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `yuangongkaohedengji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.kaoheneirong = this.ruleForm.kaoheneirong.replace(reg,'../../../ssm7w5x3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuangongkaohedengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuangongkaohedengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
