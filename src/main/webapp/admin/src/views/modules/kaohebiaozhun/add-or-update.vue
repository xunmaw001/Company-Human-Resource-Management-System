<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="考核评分" prop="kaohepingfen">
          <el-input v-model="ruleForm.kaohepingfen" 
              placeholder="考核评分" clearable  :readonly="ro.kaohepingfen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="考核评分" prop="kaohepingfen">
              <el-input v-model="ruleForm.kaohepingfen" 
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kaohebiaozhun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kaohebiaozhunCrossAddOrUpdateFlag = false;
                  this.parent.search();
