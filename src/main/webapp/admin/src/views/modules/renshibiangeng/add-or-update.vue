<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="更改时间" prop="genggaishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.genggaishijian" 
                type="datetime"
                placeholder="更改时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.genggaishijian" label="更改时间" prop="genggaishijian">
              <el-input v-model="ruleForm.genggaishijian" 
                placeholder="更改时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.genggaishijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhiwei = true;
          ...
          if(o=='genggaishijian'){
            this.ruleForm.genggaishijian = obj[o];
	    this.ro.genggaishijian = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhiweiOptions = "员工,组长,经理".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `renshibiangeng/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `renshibiangeng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.renshibiangengCrossAddOrUpdateFlag = false;
                  this.parent.search();
