<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="缺勤天数" prop="queqintianshu">
          <el-input v-model="ruleForm.queqintianshu" 
              placeholder="缺勤天数" clearable  :readonly="ro.queqintianshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="缺勤天数" prop="queqintianshu">
              <el-input v-model="ruleForm.queqintianshu" 
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `queqinkouqianbiaozhun/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `queqinkouqianbiaozhun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.queqinkouqianbiaozhunCrossAddOrUpdateFlag = false;
                  this.parent.search();
